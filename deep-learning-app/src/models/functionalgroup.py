...
def create_conv_model(input_num=input_num, addtion_num=input_num):
  ...
  x = layers.Conv1D(100, 15, activation='relu', strides=1, padding='same')(x)
...
def train_model(model):
  ...
  history = model.fit([train_data, reduce_clarity(train_data)], train_label,
                    batch_size=64,
                    epochs=40,
                    # validation_split=0.2,
                    validation_data=([validation_data, reduce_clarity(validation_data)], validation_label)
                    )
  
  return model

def evaludate_model(model):
  test_result =  model.evaluate([test_data, reduce_clarity(test_data)],  test_label, verbose=2)
  ...
  predictions = model.predict([prediction_data, reduce_clarity(prediction_data)])
  print(predictions)
